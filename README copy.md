...
This layout is essential for separating logs, application binaries, and system services to ensure stability.

| Mount Point | Purpose | Minimum Size | Notes |
| :--- | :--- | :--- | :--- |
| `/var/log` | System and Application Logs | ≥ 200 GB | **Crucial:** Prevents log growth from filling `/var`. |
| `/opt` | Application Installs & **CDP Parcel Storage** | ≥ 100 GB | Used by Cloudera parcels. |
| `/var` | System services, packages, spool, etc. | ≥ 100 GB | Heavy-write area for daemons. |
...
Used for HDFS DataNode, YARN NodeManager local, and Impala scratch directories. Constraint: Do not use disks larger than 8 TB. Total capacity per node < 100 TB.
...
### 4.4 Master Disks (HDFS High Availability)

These variables configure the storage for critical HDFS Master services, ensuring redundancy and performance isolation. 

| Variable | Service | Key Requirements |
| :--- | :--- | :--- |
| `namenode_disks` | **NameNode Metadata** | Use **multiple disks (JBOD)**. Required only on the hosts designated as NameNodes. |
| `journalnode_disks` | **JournalNode Logs** | **Dedicated disks** are strongly recommended to avoid I/O contention. |
| `checkpoint_disks` | **Checkpoints** | Typically, these are the **same disks** used by the NameNode metadata. |
